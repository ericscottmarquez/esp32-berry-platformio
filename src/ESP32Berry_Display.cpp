...
//                                  BOARD_TBOX_G03,
//                                  BOARD_BOOT_PIN};
//     static bool last_states[5] = {false, false, false, false, false};
//     bool states[5];
...
//     // Read current states
//     for (int i = 0; i < 5; i++) {
//         states[i] = digitalRead(dir_pins[i]);
//         // Check for state change
//         if (states[i] != last_states[i]) {
//             state_changed = true;
//             changed_pin = i;
//             last_states[i] = states[i]; // Update the last state
//             break; // Break after the first change detected
//         }
//     }

//     if (state_changed) {
//         // Act based on which pin changed
//         switch (changed_pin) {
//             case 0: // Right
//                 last_x = min(last_x + 5, lv_disp_get_hor_res(NULL) - mouse_cursor_icon.header.w);
//                 break;
//             case 1: // Up
//                 last_y = max(last_y - 5, 0);
//                 break;
//             case 2: // Left
//                 last_x = max(last_x - 5, 0);
//                 break;
//             case 3: // Down
//                 last_y = min(last_y + 5, lv_disp_get_ver_res(NULL) - mouse_cursor_icon.header.h);
//                 break;
//             case 4: // Button press
...
//     // Update the cursor position and button state
//     data->point.x = last_x;
//     data->point.y = last_y;
...
void Display::my_mouse_read(lv_indev_drv_t *indev_driver, lv_indev_data_t *data) {
    ...
    bool state_changed = false;
    int changed_pin = -1;
    int16_t move_amount = 25;
    int interpolation_steps = 25; // Number of intermediate steps for smoother movement
    ...
    if (state_changed) {
        int16_t target_x = last_x;
        int16_t target_y = last_y;

        // Determine the target position based on the direction
        switch (changed_pin) {
            case 0: // Right
                target_x = min(last_x + move_amount, lv_disp_get_hor_res(NULL) - mouse_cursor_icon.header.w);
                break;
            case 1: // Up
                target_y = max(last_y - move_amount, 0);
                break;
            case 2: // Left
                target_x = max(last_x - move_amount, 0);
                break;
            case 3: // Down
                target_y = min(last_y + move_amount, lv_disp_get_ver_res(NULL) - mouse_cursor_icon.header.h);
                break;
            case 4: // Button press
                left_button_down = true;
                break;
        ...
        for (int step = 1; step <= interpolation_steps; step++) {
            float fraction = (float)step / (float)interpolation_steps;
            data->point.x = last_x + (target_x - last_x) * fraction;
            data->point.y = last_y + (target_y - last_y) * fraction;
            // Normally, you would send the data point here, but since we're in a loop, we'll update the last positions at the end
        }
        
        // Update last positions to the target
        last_x = target_x;
        last_y = target_y;
    }

    // Update the cursor position and button state to their final values
    data->point.x = last_x;
    data->point.y = last_y;
    data->state = left_button_down ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
}
